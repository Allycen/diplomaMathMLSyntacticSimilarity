...
public final class FormatUtilities {
    ...
    public static int getTreeID(String s) {
        ...
        {
            int end = s.indexOf(ID_SEPARATOR, 1);
            if(end == -1)
                return -1;
            else
    ...
    public static String getRoot(String s) {
        if(s != null && s.length() > 0 && s.startsWith(OPEN_BRACKET) && s.endsWith(CLOSE_BRACKET))
        {
            int end = s.indexOf(OPEN_BRACKET, 1);
            if(end == -1)
                end = s.indexOf(CLOSE_BRACKET, 1);
